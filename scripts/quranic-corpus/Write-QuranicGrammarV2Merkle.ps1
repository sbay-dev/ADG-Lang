...
$releaseRoot = Join-Path $Root "versions\v2"
...
if (-not (Test-Path -LiteralPath $versionManifestPath)) {
    throw "Missing v2 version manifest: $versionManifestPath"
}
...
foreach ($module in $activeModules) {
    ...
    if ($dossier.moduleId -ne $module.moduleId -or $dossier.version -ne "v2") {
...
$standaloneArtifacts = @(
    ...
    @{
        ...
        Path = "scripts\quranic-corpus\Write-QuranicGrammarV2Merkle.ps1"
    }
)
...
$entries = @(
    foreach ($relative in @($artifactOwners.Keys) | Sort-Object) {
        ...
        if ($moduleId -notin $activeModuleIds) {
            throw "Artifact owner is not an active v2 module: $moduleId"
        }

        $absolute = Join-Path $Root $relative.Replace("/", "\")
        $contentSha256 = Get-NormalizedSha256 $absolute
        $descriptor = "$moduleId|v2|$relative|$contentSha256"
        [ordered]@{
            moduleId = $moduleId
            version = "v2"
            path = $relative
...
$manifest = [ordered]@{
    schemaVersion = 1
    release = "ADG-Lang Quranic Parser v2"
    version = "v2"
