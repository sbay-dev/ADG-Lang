...
$releaseRoot = Join-Path $Root "versions\v4"
...
if (-not (Test-Path -LiteralPath $versionManifestPath)) {
    throw "Missing v4 version manifest: $versionManifestPath"
}
...
foreach ($module in $activeModules) {
    ...
    if ($dossier.moduleId -ne $module.moduleId -or $dossier.version -ne "v4") {
...
$standaloneArtifacts = @(
    ...
    @{
        ...
        Path = "scripts\quranic-corpus\Write-QuranicGrammarV4Merkle.ps1"
    }
)
...
$entries = @(
    foreach ($relative in @($artifactOwners.Keys) | Sort-Object) {
        ...
        if ($moduleId -notin $activeModuleIds) {
            throw "Artifact owner is not an active v4 module: $moduleId"
        }

        $absolute = Join-Path $Root $relative.Replace("/", "\")
        $contentSha256 = Get-NormalizedSha256 $absolute
        $descriptor = "$moduleId|v4|$relative|$contentSha256"
        [ordered]@{
            moduleId = $moduleId
            version = "v4"
            path = $relative
...
$manifest = [ordered]@{
    schemaVersion = 1
    release = "ADG-Lang Quranic Parser v4"
    version = "v4"
