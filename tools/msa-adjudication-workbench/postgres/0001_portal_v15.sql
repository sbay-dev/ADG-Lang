...
CREATE TABLE IF NOT EXISTS admin_oidc_states (
...
CREATE TABLE IF NOT EXISTS submissions (
  ...
  task_version_id TEXT REFERENCES task_versions(id) ON DELETE RESTRICT,
  round_id TEXT REFERENCES consensus_rounds(id) ON DELETE RESTRICT,
  holdout_id TEXT,
...
CREATE TABLE IF NOT EXISTS result_access (
  ...
  task_version_id TEXT REFERENCES task_versions(id) ON DELETE CASCADE,
  round_id TEXT REFERENCES consensus_rounds(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS discussion_moderation (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS repository_receipts (
  id TEXT PRIMARY KEY,
  task_version_id TEXT NOT NULL
    REFERENCES task_versions(id) ON DELETE CASCADE,
  round_id TEXT NOT NULL
...
CREATE TABLE IF NOT EXISTS evidence_repository_receipts (
  id TEXT PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS governance_notification_outbox (
  id TEXT PRIMARY KEY,
  recipient_user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  event_type TEXT NOT NULL CHECK (
    event_type IN (
...
CREATE TABLE IF NOT EXISTS identity_erasure_requests (
  id TEXT PRIMARY KEY,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  status TEXT NOT NULL CHECK (
...
CREATE TABLE IF NOT EXISTS identity_erasure_items (
...
CREATE TABLE IF NOT EXISTS cpoly_runtime_state (
  singleton BOOLEAN PRIMARY KEY DEFAULT TRUE CHECK (singleton),
  current_generation BIGINT NOT NULL CHECK (current_generation >= 1),
  updated_at BIGINT NOT NULL
);

INSERT INTO cpoly_runtime_state (singleton, current_generation, updated_at)
VALUES (TRUE, 1, 0)
ON CONFLICT (singleton) DO NOTHING;

CREATE TABLE IF NOT EXISTS cpoly_write_receipts (
...
CREATE INDEX IF NOT EXISTS idx_cpoly_write_receipts_generation_seq
  ON cpoly_write_receipts(generation, receipt_seq);
