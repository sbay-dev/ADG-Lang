    ...
    bool PrintRendered)
{
    public static CompileOptions Parse(string[] args)
    {
        ...
        for (var i = 1; i < args.Length; i++)
        {
            switch (args[i])
            {
                ...
                case "--native":
                    ...
                    break;
                case "--wasi":
                    wasi = RequireValue(args, ref i, "--wasi");
                    break;
                case "--wasi-sysroot":
                    wasiSysroot = RequireValue(args, ref i, "--wasi-sysroot");
                    break;
                case "--clang":
        ...
        return new CompileOptions(input, llvm, native, wasi, wasiSysroot, clang, print);
    }
