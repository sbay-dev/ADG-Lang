...
internal static class AdgJsonParser
{
    public static IAdgNode Parse(JsonElement element)
    {
        ...
        return Normalize(kind) switch
        {
            ...
            "hiddenreference" or "hiddenref" => ParseHiddenReference(element),
    ...
    private static HiddenReference ParseHiddenReference(JsonElement element)
    {
        IAdgNode? target = null;
        if (element.TryGetPropertyAny(out var targetElement, "target", "antecedent", "referent"))
        {
            target = Parse(targetElement);
        }

        var surface = GetOptionalString(element, "surface", "text", "reference") ?? "HiddenReference";
        var inferred = GetOptionalBool(element, true, "inferred");
        var certainty = GetOptionalString(element, "certainty") ?? "low";
        var resolved = GetOptionalBool(element, target is not null, "resolved", "isResolved");
        return new HiddenReference(surface, resolved ? target : null, inferred, certainty);
    }
    ...
    private static AdgCase ParseCase(string value) => Normalize(value) switch
    {
        ...
        "raf" or "رفع" or "rafu" or "مرفوع" => AdgCase.Raf,
        "nasb" or "نصب" or "منصوب" => AdgCase.Nasb,
        "jarr" or "جر" or "jar" or "مجرور" => AdgCase.Jarr,
    ...
    private static bool GetOptionalBool(JsonElement element, bool defaultValue, params string[] names)
    {
        if (!element.TryGetPropertyAny(out var property, names))
        {
            return defaultValue;
        ...
        return property.ValueKind switch
        {
            ...
            _ => throw new AdgParseException($"Property '{names[0]}' must be a boolean.")
        };
    }
    ...
    private static string Normalize(string value)
    {
        ...
        {
            if (char.IsWhiteSpace(ch) || ch is '_' or '-' or '‘' or '’' or '\'' or '`' or 'ء' || ch is >= '\u064B' and <= '\u065F' or '\u0670')
            {
                continue;
            }
