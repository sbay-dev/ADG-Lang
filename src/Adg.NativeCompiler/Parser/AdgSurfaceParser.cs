...
internal static partial class AdgSurfaceParser
{
    ...
    [GeneratedRegex("^جارٌّ\\s+ومجرورٌ\\s+\"(?<operator>[^\"]+)\"\\s+إضافةٌ\\s+\"(?<mudaf>[^\"]+)\"\\s+(?<mudafCase>\\S+)\\s+\"(?<mudafIlayh>[^\"]+)\"\\s+(?<mudafIlayhCase>\\S+)\\s*$", RegexOptions.CultureInvariant)]
